...
export function StudentLiveExperience({ lecture, live }: { lecture: Lecture; live: ReturnType<typeof useLiveSession> }) {
  ...
  return <main className={`slide-screen learn-shell lb-motion-root ${round ? "question-open" : ""}`} data-live-status={live.state?.status ?? "connecting"}>
    <SlideEngineCanvas mobileReading lectureToken={lecture.publicToken} participationPath={lecture.participationPath} lectureTitle={lecture.title} showJoinIntro={live.state?.showIntro ?? true}
