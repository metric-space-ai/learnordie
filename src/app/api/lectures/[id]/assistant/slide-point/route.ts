...
const slidePointSchema = z.object({
...
export async function POST(request: Request, context: { params: Promise<unknown> }) {
  ...
  if (!bodyResult.ok) {
    return NextResponse.json({ error: "Folienpunkt konnte nicht gelesen werden." }, { status: bodyResult.status });
  }

  const parsed = slidePointSchema.safeParse(bodyResult.body);
  if (!parsed.success) {
    return NextResponse.json({ error: "Folienpunkt konnte nicht übernommen werden." }, { status: 400 });
  }

  const repository = getLectureRepository();
  const lecture = await repository.applyLecturerAssistantSlidePoint({
    lectureId: id,
