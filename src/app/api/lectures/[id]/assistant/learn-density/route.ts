...
const learnDensitySchema = z.object({
...
export async function POST(request: Request, context: { params: Promise<unknown> }) {
  ...
  if (!bodyResult.ok) {
    return NextResponse.json({ error: "Learn-Fragedichte konnte nicht gelesen werden." }, { status: bodyResult.status });
  }

  const parsed = learnDensitySchema.safeParse(bodyResult.body);
  if (!parsed.success) {
    return NextResponse.json({ error: "Learn-Fragedichte konnte nicht gesetzt werden." }, { status: 400 });
  }

  const repository = getLectureRepository();
  const lecture = await repository.applyLecturerAssistantLearnDensity({
    lectureId: id,
