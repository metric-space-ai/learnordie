...
import { savePrivateWordingBackup } from "./lib/private-wording-backup.mjs";

const args = process.argv.slice(2);
if (args.includes("--help") || args.includes("-h")) {
  console.log("Usage: node scripts/apply-question-wording.mjs --plan PRIVATE.json --owner-email OWNER --public-token TOKEN [--apply --confirm-sha256 FILE_SHA256 (--backup DURABLE_NEW_FILE.json | --backup-blob maintenance/question-wording/UNIQUE.json)]\nUses DATABASE_URL; default is a locked, owner-scoped dry-run. Application requires the exact file hash and an exclusive, durable backup. Private Blob backup is available inside Vercel production and must pass read-back and anonymous-access checks. End the live session first. No authentication/provider bypass is provided.");
} else {
  let sql;
  try {
    const values = {};
    for (let i = 0; i < args.length; i += 1) {
      ...
      if (!["--plan", "--owner-email", "--public-token", "--confirm-sha256", "--backup", "--backup-blob"].includes(flag) || values[flag] !== undefined || !args[i + 1] || args[i + 1].startsWith("--")) throw new Error("Invalid arguments; use --help");
      values[flag] = args[++i];
    }
    ...
    if (values.apply && (values["--confirm-sha256"] !== fileSha256 || Number(Boolean(values["--backup"])) + Number(Boolean(values["--backup-blob"])) !== 1)) throw new Error("Application requires matching file SHA256 and exactly one new backup path");
    if (values["--backup-blob"] && process.env.VERCEL_ENV !== "production") throw new Error("Private Blob maintenance requires Vercel production execution");
    ...
    const result = await applyQuestionWording(sql, JSON.parse(content), {
      ...
      backup: async (backup) => {
        if (values["--backup-blob"]) {
          backupReceipt = await savePrivateWordingBackup(values["--backup-blob"], { ...backup, fileSha256 });
          return;
        }
    ...
    console.log(JSON.stringify({ ...result, fileSha256, ...(result.status === "applied" ? { backupPath, backupReceipt } : {}) }));
  } catch (error) {
